...
namespace SoftKitty.MasterNavigationMap
{
    ...
    public class UILineRenderer : MaskableGraphic
    {
        ...
        public void SetPositions(Vector3[] _points)
        {
            points = new Vector2[_points.Length];
            for (int i = 0; i < points.Length; i++)
            {
                points[i] = new Vector2(_points[i].x, _points[i].z);
            }
            SetVerticesDirty();
        }

        public void SetPosition(int _index, Vector3 _point)
        {
            points[_index] = new Vector2(_point.x, _point.z);
            SetVerticesDirty();
        }
